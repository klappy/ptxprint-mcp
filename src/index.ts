...
// ---------- Env ----------
//
// Extends `Cloudflare.Env` (generated by `wrangler types` from wrangler.jsonc).
// The generated type carries every binding declared in wrangler.jsonc with
// the most precise type wrangler knows — including literal types for `vars`
// (e.g. WORKER_URL: "https://ptxprint.klappy.dev"). Bindings declared in
// wrangler.jsonc do NOT need to be re-listed here; doing so previously caused
// a literal-vs-string type mismatch that made `tsc --noEmit` fail on main.
//
// What this interface adds: secrets. Wrangler's typegen knows nothing about
// `wrangler secret put` values because they don't appear in wrangler.jsonc,
// so the secret bindings are declared here.
//
// Regenerate the underlying typegen after wrangler.jsonc changes:
//   npx wrangler types
