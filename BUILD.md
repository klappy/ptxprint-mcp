...
Deploy effect since 0.2.0: the Worker serves `docs()` from `src/bundled-canon.ts`, generated by `npm run bundle-canon` from `canon/{articles,governance,specs,templates,handoffs}` + `canon/README.md`. **Every canon edit must be followed by `npm run bundle-canon` and the regenerated file committed** — CI fails on a stale bundle. Push to main deploys the new bundle like any code change. Agents that read the canon through their own `oddkit` (`knowledge_base_url=https://github.com/klappy/ptxprint-mcp`) see the change on push, subject to oddkit's caching.
