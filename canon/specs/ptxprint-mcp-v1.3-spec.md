...
Same return shape, plus `served_from` (the canon bundle hash and doc count).

**2026-09-05 amendment (draft spec, edited in place per CONTRIBUTING; Worker 0.2.0).** No longer forwards to oddkit. The Worker searches `src/bundled-canon.ts` — generated by `scripts/bundle-canon.ts` from `canon/{articles,governance,specs,templates,handoffs}` + `canon/README.md` — with in-process BM25 (title bonus; audience tags as tiebreaker). Depth 1 = snippet + five sources; depth 2 = full top doc; depth 3 = full top three. `governance_source` is always `"knowledge_base"`; the `"minimal"` fallback is retired because there is no upstream to be unreachable. Why: the oddkit proxy read `result.hits`; oddkit's retrieval-disclosure contract moved results to `result.data`, and every query returned empty for an unknown span (found by validation 2026-09-05, row D1). Ruling (captain, 2026-09-04): docs tools manage and serve their own policy. CI fails when the bundle is stale.
